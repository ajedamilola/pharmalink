...
const PharmacyMarketplace = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      let query = supabase
        ...
        .select(
          "*, drugs(*), vendors(name, verification_status), pharmacies(name)"
        )
        .eq("status", "active")
      ...
      const filtered = (data || [])
        .filter(
          (l) =>
            l.source === "buyback" ||
            !l.vendor_id ||
            l.vendors?.verification_status === "verified"
        )
        // Deduplicate: keep only the latest listing per drug+pharmacy+source+price combo
        .reduce((acc: any[], listing) => {
          const key = `${listing.drug_id}-${listing.pharmacy_id ?? listing.vendor_id}-${listing.source}-${listing.unit_price}`;
          const exists = acc.find(
            (l) =>
              `${l.drug_id}-${l.pharmacy_id ?? l.vendor_id}-${l.source}-${l.unit_price}` ===
              key
          );
          if (!exists) acc.push(listing);
          return acc;
        }, []);
  ...
  const addToCart = (listing: any) => {
    ...
    if (existing) {
      setCart(
        cart.map((c) =>
          c.listing.id === listing.id ? { ...c, quantity: c.quantity + 1 } : c
        )
      );
    } else {
  ...
  const updateCartQuantity = (listingId: string, quantity: number) => {
    ...
    setCart((prev) =>
      prev.map((c) => (c.listing.id === listingId ? { ...c, quantity } : c))
    );
  };

  const cartSubtotal = cart.reduce(
    (sum, c) => sum + c.listing.unit_price * c.quantity,
    0
  );
  const logisticsFee =
    Math.round(cartSubtotal * LOGISTICS_FEE_RATE * 100) / 100;
  ...
  const checkout = async () => {
    ...
    if (Number(ph.wallet_balance) < cartTotal) {
    ...
    for (const item of cart) {
      ...
      const itemLogistics =
        Math.round(itemSubtotal * LOGISTICS_FEE_RATE * 100) / 100;
    ...
    await supabase
      .from("pharmacies")
      .update({ wallet_balance: Number(ph.wallet_balance) - cartTotal })
      .eq("id", ph.id);
          ...
          <Card key={listing.id}>
            <CardHeader className='pb-2'>
              <div className='w-full flex justify-center'>
                {getDrugImageUrl(listing.drug_id) ? (
                  <img
                    src={getDrugImageUrl(listing.drug_id) || ""}
                    alt={listing.drugs?.name}
              ...
              <div className='flex items-start justify-between'>
                ...
                <Badge
                  variant={
                    listing.source === "buyback" ? "secondary" : "default"
                  }
                >
                  {listing.source}
                </Badge>
            ...
            <CardContent className='space-y-3'>
              <div className='text-2xl font-bold'>
                ₦{Number(listing.unit_price).toLocaleString()}
              </div>
              {listing.discount_pct > 0 && (
                <Badge variant='secondary'>{listing.discount_pct}% off</Badge>
              )}
              <div className='text-sm text-muted-foreground space-y-1'>
                ...
                <p>
                  From:{" "}
                  {listing.vendors?.name || listing.pharmacies?.name || "—"}
                </p>
              </div>
              <Button
                size='sm'
              ...
              >
                <div>
                  <p className='font-medium text-sm'>
                    {item.listing.drugs?.name}
                  </p>
                  <p className='text-xs text-muted-foreground'>
                    ₦{Number(item.listing.unit_price).toLocaleString()} each
                  </p>
                </div>
                <div className='flex items-center gap-2'>
                  ...
                  <span className='font-medium whitespace-nowrap'>
                    ₦
                    {(item.listing.unit_price * item.quantity).toLocaleString()}
                  </span>
                  <Button
                    variant='ghost'
                    size='icon'
