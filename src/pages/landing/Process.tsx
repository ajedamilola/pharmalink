...
export default function Process() {
  return (
    <section id="process" className="bg-[#050505] text-white py-24 px-4 md:px-12 font-[arimo]">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {STEPS.map((step, index) => (
            <div key={index} className="group relative p-8 rounded-4xl bg-white/5 border border-white/10 hover:border-[#22c55e]/50 transition-all duration-500 hover:-translate-y-2">
              <div className="w-14 h-14 bg-[#22c55e] text-black rounded-2xl flex items-center justify-center mb-8 rotate-3 group-hover:rotate-0 transition-transform">
                {step.icon}
              </div>
              <div className="absolute top-8 right-8 text-white/10 text-6xl font-black select-none group-hover:text-[#22c55e]/10 transition-colors">
                0{index + 1}
              </div>
