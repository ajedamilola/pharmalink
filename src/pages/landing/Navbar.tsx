...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`flex items-center justify-between py-4 fixed top-0 left-0 right-0 z-50 max-w-[1400px] mx-auto w-full px-4 md:px-8 transition-transform duration-300 ${isVisible || isOpen ? "translate-y-0 bg-white" : "-translate-y-full"}`}
      >
        ...
        <div className='hidden md:flex items-center gap-4'>
          ...
          <Link
            to='/login'
            className='bg-[#22c55e] hover:bg-[#16a34a] text-green-950 px-8 py-3 rounded-full text-lg font-bold transition-all shadow-[0_0_20px_rgba(204,255,0,0.2)]'
          >
            Login
          </Link>
        </div>
